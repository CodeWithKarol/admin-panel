...
import { TeamMember, STATUS_COLORS } from '../../../../core/models/analytics.models';
      ...
      @for (member of members(); track member.id) {
        ...
        >
          ...
          <div class="relative aspect-[3/4] mb-6 overflow-hidden bg-brand-100 rounded-sm">
            ...
            <div
              class="absolute top-4 right-4 px-3 py-1 bg-white/90 backdrop-blur-sm border border-brand-100 rounded-full shadow-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300"
            >
              <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-full" [ngClass]="getStatusColor(member.status)"></div>
                <span class="text-[10px] font-black uppercase tracking-tighter text-brand-600">{{
                  member.status
                }}</span>
              </div>
            </div>
            ...
            <!-- Action Prompt -->
            <div
              class="absolute inset-0 bg-accent-charcoal/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none"
            >
              <lucide-icon
                [name]="ExternalLink"
                class="w-8 h-8 text-white stroke-[1px]"
              ></lucide-icon>
            </div>
          </div>
          ...
          <div class="space-y-3">
            <div class="flex justify-between items-start">
              <h3
                class="font-serif text-2xl font-bold text-accent-charcoal group-hover:text-accent-terracotta transition-colors"
              >
                {{ member.name }}
              </h3>
            ...
            <div
              class="w-full h-[1px] bg-brand-100 group-hover:bg-accent-terracotta transition-colors duration-500"
            ></div>

            <div
              class="flex justify-between items-center pt-1 text-[9px] font-bold text-brand-400 uppercase tracking-widest"
            >
...
export class UserListComponent {
  ...
  getStatusColor(status: TeamMember['status']) {
    return STATUS_COLORS[status] || 'bg-brand-200';
  }

  openDossier(member: TeamMember) {
    this.inspector.open(this.analyticsService.getMemberDossier(member), 'user');
  }
}
